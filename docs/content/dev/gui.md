...
- **Site pages** (`sitePaths`: the home and formats) are for Google: complete in the server's HTML
  ...
  Their frame is `SiteShell` in [`pages.tsx`](https://github.com/joeblew999/remy-auth/blob/main/packages/ui/src/pages.tsx), built from static
  shadcn parts.
...
- `src/parts.json`: the package's [parts](https://github.com/joeblew999/remy-auth/blob/main/.plans/done/parts.md) this app uses, one name per line (today `time-zones`); `remyParts()` in `vite.config.ts` mounts their routes and `partChecks()` in `tests/gui.spec.ts` runs their checks.
