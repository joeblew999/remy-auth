...
remy-auth adds its own site pages ([`src/paths.ts`](../src/paths.ts)): the docs at `/docs`, in
English only, that render this repository's Markdown in place, and beside them in every language the
docs search `/docs/search` and `/docs/ask`, which answers questions from the docs with links to the
pages and sections it used (the old app page `/app/ask` redirects there). The
[docs site plan](../.plans/done/docs-site.md) and [the answers plan](../.plans/done/docs-ai-sync.md) own them.
...
| `/en/app` | App | Live status card: TanStack Query over the contract's `GET /api/status` (the router itself during SSR, HTTP in the browser), server-rendered, polled every 10 s, with a refresh that invalidates every loader and query at once (`invalidateEverything` in `@joeblew999/remy-ui/invalidate`); the card itself is the package's `showcase/status-card`, which remy-auth-app shows too, asked across origins (CORS for its registered origin, `src/api/origins.ts`) |
...
- `src/parts.json`: the package's [parts](../.plans/done/parts.md) this app uses, one name per line (today `time-zones`); `remyParts()` in `vite.config.ts` mounts their routes and `partChecks()` in `tests/gui.spec.ts` runs their checks.
