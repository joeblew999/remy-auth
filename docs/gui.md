...
- `src/parts.json`: the package's [parts](../.plans/parts.md) this app uses, one name per line (today `time-zones`); `remyParts()` in `vite.config.ts` mounts their routes and `partChecks()` in `tests/gui.spec.ts` runs their checks.
