// Written by `mise run docs:cli -- feature webmcp`. Adapted: one per docs site (its search API and its
// language are props), and page URLs map to Markdown as the site serves it (/docs/es/formats.md).
...
/** the Markdown URL of a page, e.g. `/docs/page` -> `/docs/page.md` */
function markdownUrl(url: string) {
  const path = url.split('#')[0]!.replace(/\/$/, '');
  return /^\/(docs|dev)(\/[a-z]{2,3}(-[A-Za-z]{2,4})?)?$/.test(path) ? `${path}/index.md` : `${path}.md`;
}

/** expose the docs to AI agents in the browser */
export function WebMCP({ site, lang: locale }: { site: string; lang: string }) {
  ...
  useEffect(() => {
    ...
    const client = fetchClient({ api: `/api/search/${site}`, locale });
