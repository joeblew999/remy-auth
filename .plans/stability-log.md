...
| 09-26 | The docs Worker's build output (1,102 files) was committed | root-anchored ignore rules (`/dist/`) vs a new workspace (`docs/`) | leaked, found in the cleanup | `/docs/dist/`, `/docs/worker-configuration.d.ts` ignored; the branch is squash-merged so the files never reach main's history |
| 09-26 | Canonical, hreflang and og:image pointed at `http://fallback.com` | a Paraglide helper (`getUrlOrigin`) used in a Worker without Paraglide | leaked, caught by fetching as Googlebot and Lighthouse SEO (92) | the request's own origin (TanStack Start `getRequest()`); SEO 100 |
| 09-26 | The shared smoke tier ran none of the package's smoke checks | a file filter (`tests/smoke.spec.ts`) vs checks defined in the package's `smoke.js` | leaked since the tier existed: only the app's in-file test ran | select by title (`smoke <locale>:`) |
| 09-26 | Every app test file failed to load | a component module (`import.meta.env`) imported by a Node test | leaked, caught by running the tier | the docs origin in its own plain module |
| 09-26 | `cf:ai-check` read AI Search with the API token (403) | `fnox exec` sets `CLOUDFLARE_API_TOKEN`, which Wrangler prefers over its login | leaked (tooling) | Wrangler runs without it; the token is used only for Logs and AI Gateway |
| 09-25 | The recipe agent's English `docs/content/dev/tasks.md` edit left the Spanish behind again (1 failure) | translations vs their source, second time | leaked, caught by the full run | `docs:translations` in tier 0 (`project:check`) now catches it before merge |
