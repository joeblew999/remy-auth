...
| 4. Display time zone and location | Owner decision 2026-09-24: use Cloudflare's request geolocation. The Worker passes `request.cf` through a typed React Router context (`app/context.ts`), and server rendering shows the visitor's country, city, region, time zone and local time from it, labelled as the network's location; it is never logged or stored, and personalised pages already send `Cache-Control: no-store`. Catalog dates still format in UTC with the zone labelled, and the device's own zone renders in the browser after hydration. A stored user or organisation preference can later override both. |
...
| 9. Beyond dates, numbers and currency | Taking visitors to their language follows [Google's multilingual guidance](https://developers.google.com/search/docs/specialty/international/managing-multi-regional-sites): "Avoid automatically redirecting users from one language version of a site to a different language version", "Don't use IP analysis to adapt your content", and offer links to the other versions; `x-default` "was designed for language selector pages". Revised 2026-09-24 under the owner's rule against reinventing wheels: Paraglide's own `url`, `cookie`, `preferredLanguage` and `baseLocale` strategies and its middleware handle detection, the cookie and the redirect of un-localized entry URLs (`/`, `/demo`, `/formats`), which are the `x-default` targets; localized pages never redirect and show a dismissible hint instead. Every localized URL stays explicit and prefixed. The formats page also shows compact numbers, units, currencies with different minor units, ordinals, value variants, interpolation, locale-aware sorting, region and currency names and date ranges; the demo has a localized form with validation and a plural confirmation; the sitemap carries `hreflang` alternates. Segmenter and DurationFormat wait for Baseline widely available. |
