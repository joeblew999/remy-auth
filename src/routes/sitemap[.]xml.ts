...
// then the docs, English only: their /en URL alone, without alternates (.plans/docs-site.md, decision 3).
  ...
  server: {
    handlers: {
      GET: ({ request }) => {
        ...
        const docs = docsPaths.map(path => `<url><loc>${origin}/${docsLocale}${path}</loc></url>`);
